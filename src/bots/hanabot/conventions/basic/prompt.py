...
class Prompt(Convention):
    ...
    def find_interpretation_for_clue_to_others(self, turn: Turn, focus_card: HandCard) -> Interpretation | None:
        ...
        if not not_clued_missing_cards_to_play:
            visible_prompted_cards = [turn.game_state.find_clued(card) for card in missing_cards_to_play]
        ...
        probable_missing_card = self.find_most_probables(not_clued_missing_cards_to_play, turn.game_state.my_hand)
    ...
    def find_interpretation_for_clue_to_me(self, turn: Turn, focus_card: HandCard) -> Interpretation | None:
        if isinstance(turn.action, SuitClueAction):
            ...
            for card in reversed(turn.game_state.stacks.cards_left_to_play_on(turn.action.suit)):
                cards_leading_to = turn.game_state.find_clued_cards_leading_to(card)
            ...
            return Interpretation(
                ...
                interpretation_type=InterpretationType.PLAY,
                explanation=self.name,
                notes_on_cards={**{card.draw_id: {card.real_card} for _, _, card in cards_leading_to}, **{focus_card.draw_id: {highest_possible_card}}},
            )
