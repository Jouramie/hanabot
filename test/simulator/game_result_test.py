...
def test_gamestate_with_three_strikes_should_be_loss_result():
    gamestate = GameState(get_player_names(5), get_suits(5))
    gamestate.stacks[Suit.RED].last_played = Card(Suit.RED, Rank.TWO)
...
def test_gamestate_with_one_strike_should_be_result_with_score():
    gamestate = GameState(get_player_names(5), get_suits(5))
    gamestate.stacks[Suit.RED].last_played = Card(Suit.RED, Rank.THREE)
    gamestate.stacks[Suit.BLUE].last_played = Card(Suit.BLUE, Rank.FIVE)
    gamestate.stacks[Suit.YELLOW].last_played = Card(Suit.YELLOW, Rank.FOUR)
...
def test_gamestate_with_one_strike_should_be_victory_result_with_max_score():
    gamestate = GameState(get_player_names(5), get_suits(5))
    gamestate.stacks[Suit.RED].last_played = Card(Suit.RED, Rank.FIVE)
    gamestate.stacks[Suit.BLUE].last_played = Card(Suit.BLUE, Rank.FIVE)
    gamestate.stacks[Suit.YELLOW].last_played = Card(Suit.YELLOW, Rank.FIVE)
    gamestate.stacks[Suit.GREEN].last_played = Card(Suit.GREEN, Rank.FIVE)
    gamestate.stacks[Suit.PURPLE].last_played = Card(Suit.PURPLE, Rank.FIVE)
    gamestate.add_strike()
    gamestate.is_over = True
